...
run "fast_to_get_going" {
  ...
  assert {
    condition     = azurerm_windows_function_app.this["func-app-ldo-uks-tst-01"].storage_uses_managed_identity == true
    error_message = "Keyless apps should authenticate host storage with the managed identity."
  }

  assert {
    condition     = azurerm_windows_function_app.this["func-app-ldo-uks-tst-01"].storage_account_access_key == null
    error_message = "No access key should be passed keyless."
  }

  assert {
    condition     = azurerm_windows_function_app.this["func-app-ldo-uks-tst-01"].app_settings["AzureWebJobsStorage__clientId"] == "00000000-0000-0000-0000-00000000bbbb"
  ...
  assert {
    condition     = azurerm_windows_function_app.this["func-app-ldo-uks-tst-01"].identity[0].type == "SystemAssigned, UserAssigned"
    error_message = "The module identity default should attach both kinds."
  }

  assert {
    condition     = azurerm_windows_function_app.this["func-app-ldo-uks-tst-01"].https_only == true
    error_message = "https_only should default true (the provider default is false)."
  }

  assert {
    condition     = azurerm_windows_function_app.this["func-app-ldo-uks-tst-01"].ftp_publish_basic_authentication_enabled == false && azurerm_windows_function_app.this["func-app-ldo-uks-tst-01"].webdeploy_publish_basic_authentication_enabled == false
    error_message = "Basic-auth publishing should default off (the provider default is on)."
  }

  assert {
    condition     = azurerm_windows_function_app.this["func-app-ldo-uks-tst-01"].builtin_logging_enabled == false
    error_message = "Builtin logging should default off (App Insights supersedes the dashboard)."
  }

  assert {
    condition     = azurerm_windows_function_app.this["func-app-ldo-uks-tst-01"].functions_extension_version == "~4"
...
run "plan_shapes" {
  ...
  assert {
    condition     = azurerm_windows_function_app.this["func-c-ldo-uks-tst-01"].service_plan_id == "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-x/providers/Microsoft.Web/serverFarms/asp-byo"
...
run "byo_storage_by_id" {
  ...
  assert {
    condition     = azurerm_windows_function_app.this["func-byo-ldo-uks-tst-01"].storage_account_name == "stshared"
...
run "key_vault_secret_shape" {
  ...
  assert {
    condition     = azurerm_windows_function_app.this["func-kv-ldo-uks-tst-01"].storage_key_vault_secret_id == "https://kv-mock.vault.azure.net/secrets/func-storage/0000"
    error_message = "The Key Vault secret id should pass through."
  }

  assert {
    condition     = azurerm_windows_function_app.this["func-kv-ldo-uks-tst-01"].storage_account_name == null && azurerm_windows_function_app.this["func-kv-ldo-uks-tst-01"].storage_uses_managed_identity == null
...
run "keys_on_opt_out" {
  ...
  assert {
    condition     = azurerm_windows_function_app.this["func-keys-ldo-uks-tst-01"].storage_account_access_key != null && azurerm_windows_function_app.this["func-keys-ldo-uks-tst-01"].storage_uses_managed_identity == null
...
run "no_identity_at_all" {
  ...
  assert {
    condition     = length(azurerm_windows_function_app.this["func-noid-ldo-uks-tst-01"].identity) == 0
...
run "app_insights_wiring" {
  ...
  assert {
    condition     = azurerm_windows_function_app.this["func-ai-ldo-uks-tst-01"].app_settings["APPLICATIONINSIGHTS_CONNECTION_STRING"] != ""
    error_message = "The AI connection string setting should be wired."
  }

  assert {
    condition     = azurerm_windows_function_app.this["func-ai-ldo-uks-tst-01"].app_settings["APPLICATIONINSIGHTS_AUTHENTICATION_STRING"] == "ClientId=00000000-0000-0000-0000-00000000bbbb;Authorization=AAD"
...
run "zip_deploy_pairing_accepted" {
  ...
  assert {
    condition     = azurerm_windows_function_app.this["func-zip-ldo-uks-tst-01"].zip_deploy_file == "app.zip"
