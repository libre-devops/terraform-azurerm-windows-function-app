...
The one-entry PowerShell call: a runtime-only entry gets a dedicated Y1 consumption plan, keyless storage, a pre-granted user-assigned identity, and identity-based host storage.
...
# CI run; the PowerShell app/ package is pushed by the CI deploy stage.
locals {
  location  = lookup(var.regions, var.loc, "uksouth")
  rg_name   = "rg-${var.short}-${var.loc}-${terraform.workspace}-001"
  func_name = "func-wfa-${var.short}-${var.loc}-${terraform.workspace}-001"
}
...
output "default_hostname" {
  value = module.windows_function_app.default_hostnames[local.func_name]
}
