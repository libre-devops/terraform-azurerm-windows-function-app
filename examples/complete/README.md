...
A shared B1 plan hosting a keyless PowerShell API (App Insights with AAD ingestion, always_on, health checks, CORS, TLS 1.3) next to a keys-on node worker with a system-assigned identity.
...
locals {
  ...
  api_name  = "func-wapi-${var.short}-${var.loc}-${terraform.workspace}-002"
  wkr_name  = "func-wwkr-${var.short}-${var.loc}-${terraform.workspace}-002"
...
module "windows_function_app" {
  ...
  function_apps = {
    ...
    (local.api_name) = {
      ...
      site_config = {
        always_on                         = true
        health_check_path                 = "/api/HttpTrigger"
...
output "api_default_hostname" {
  value = module.windows_function_app.default_hostnames[local.api_name]
}
